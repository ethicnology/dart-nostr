...
import 'package:nostr/src/namecoin/record_parser.dart';
import 'package:nostr/src/namecoin/tlsa.dart';
...
class RelayResolution {
  ...
  /// TLSA records published alongside the relay, in declaration
  /// order. Empty if the record has no `tls` field.
  ///
  /// Use [TlsaRecord.matchesCertificate] / [TlsaRecord.matchesSpki]
  /// inside an `HttpClient`'s `badCertificateCallback` (or the
  /// equivalent on your platform's TLS stack) to pin the relay
  /// handshake.
...
class NamecoinRelayResolver {
  ...
  Future<RelayResolution?> resolve(String wssUrl) async {
    ...
    final tlsaRecords =
        parseTlsaRecords(valueJson, hostFlat.subdomainLabels);
...
class _CachedResolution {
  ...
  factory _CachedResolution.hit({
    ...
    required List<TlsaRecord> tlsaRecords,
