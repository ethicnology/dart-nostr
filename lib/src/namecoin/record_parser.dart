...
/// Parses the `tls` field as a list of [TlsaRecord]s, optionally
/// walking into a subdomain first. Malformed entries are skipped.
List<TlsaRecord> parseTlsaRecords(
  String rawValueJson, [
  List<String> subdomainLabels = const [],
]) {
  final root = _decodeJsonObject(rawValueJson);
  ...
  final target = walkSubdomain(root, subdomainLabels);
  if (target == null) return const [];
  return _collectTlsaRecords(target);
}

List<TlsaRecord> _collectTlsaRecords(Map<String, dynamic> obj) {
  final tls = obj['tls'];
  if (tls is! List) return const [];
  final out = <TlsaRecord>[];
  for (final entry in tls) {
    if (entry is! List) continue;
    final record = TlsaRecord.tryParse(entry);
